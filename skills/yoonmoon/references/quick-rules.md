...
**사용자 선호 오버라이드:** 의미가 보존되고 장르와 충돌하지 않으면 (1) 수동태는 능동태로 바꾼다(`쓰인다` → `쓴다`), (2) 운영 문서의 과한 구어체·거친 표현은 순화한다(`죽는다` → `멈춘다`, `잡는다` → 문맥에 따라 `설정한다`/`연결한다`), (3) 과하게 압축된 한자어는 풀어 쓴다(`고권한`/`고권` → `높은 권한`), (4) 영어 에세이 톤의 직역체는 기술 블로그처럼 짧고 건조하게 압축한다.
...
6. **사용자 선호 반영**: 수동태→능동태, 과한 구어체 순화, 과한 한자어 풀어쓰기, 영어 에세이식 직역체 압축을 의미 보존 범위에서 적용했는가

...
**사용자 선호 오버라이드:** 의미가 보존되고 장르와 충돌하지 않으면 (1) 수동태는 능동태로 바꾼다(`쓰인다` → `쓴다`), (2) 운영 문서의 과한 구어체·거친 표현은 순화한다(`죽는다` → `멈춘다`, `잡는다` → 문맥에 따라 `설정한다`/`연결한다`), (3) 과하게 압축된 한자어는 풀어 쓴다(`고권한`/`고권` → `높은 권한`), (4) 영어 에세이 톤의 직역체는 기술 블로그처럼 짧고 건조하게 압축한다, (5) 핵심 동작·결론은 문장 앞에 둔다(`세션을 완전히 끝내지 않고, 백그라운드로 분리합니다.` → `세션을 백그라운드로 분리합니다. 세션을 끝내지 않습니다.`).
...
| A-9a | 행위자가 뚜렷한 일반 수동태 | S2 | 능동태 우선("쓰인다" → "쓴다", "사용된다" → "사용한다") |
...
| A-20 | 영어 에세이식 직역 프레이밍 | S2 | 짧은 기술 블로그 문장으로 압축. `~라는 관점`, `문제의식`, `유용하다`, `실생활`, `충분하지 않다` 금지. `핵심은 ~이다` 수준까지만 허용 |
...
| F-6 | 과하게 압축된 한자어·전문어 | S2 | 쉬운 한국어로 풀어쓰기("고권한/고권" → "높은 권한") |

## F2. 운영 문서 구어체 순화

| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| O-1 | 거칠거나 과한 구어체 동사 | S2 | 문서·강의노트·런북에서는 차분한 동사로 순화("죽는다" → "멈춘다", "잡는다" → 문맥상 "설정한다"/"연결한다") |
...
| H-4 | "즉" 남발 | S2 | 1회로 제한 |
| H-5 | 설명조 에세이 연결어 | S2 | 불필요한 `그래서`, `다만`, `이때`, `이러한`, `이를 통해` 제거. 문장 순서로 논리를 보이게 함 |
| H-6 | 핵심 동작·결론을 뒤로 미루는 종속절 | S2 | 두괄식으로 분리. 먼저 핵심 동작을 말하고, 조건·부연은 다음 문장에 둔다 |
...
6. **사용자 선호 반영**: 수동태→능동태, 과한 구어체 순화, 과한 한자어 풀어쓰기, 영어 에세이식 직역체 압축, 핵심 동작·결론의 두괄식 분리를 의미 보존 범위에서 적용했는가
7. **인공 표현 자제**: 원문에 없던 비유·수사·문학적 표현을 윤문 과정에서 임의로 추가하지 않았는가

...
7. **사용자 선호 반영(User Overrides)**: 의미가 같다면 수동태보다 능동태, 거친 구어체보다 차분한 운영 문서 어휘, 압축된 한자어보다 풀어 쓴 쉬운 표현을 우선한다. 영어 에세이 톤의 직역체는 짧고 건조한 기술 블로그 문체로 압축한다. 핵심 동작이나 결론은 앞에 두고, 부연은 뒤로 분리한다.
...
- 금지 표현: `~라는 관점`, `문제의식`, `유용하다`, `실생활`, `충분하지 않다`.
- 설명조 에세이 문체를 피하고, 기술 블로그처럼 건조하게 쓴다.
- 문장 길이를 줄인다. 한 문장에 배경·평가·결론을 모두 넣지 않는다.
- 추상명사보다 동사를 쓴다: `접근성 확보`보다 `접속한다`, `활용성 향상`보다 `더 자주 쓴다`.
- `핵심은 ~이다` 수준의 짧은 요약은 허용한다. 그 뒤에는 바로 구체 동작을 쓴다.
- 불필요한 연결어와 번역투를 제거한다.

| 원문 패턴 | 윤문 예시 |
|-----------|----------|
| 이 강의의 문제의식은 단순하다 | 이 강의의 핵심은 ~이다 |
| X만으로는 충분하지 않다 | X해도 Y하기 어렵다 / X만으로는 어렵다 |
| 실생활 속에서 꾸준히 쓰기 어렵다 | 일상적으로 쓰기 어렵다 |
| 유용해진다는 관점에서 | 쓰기 위해 / 쓰려면 |
| A라는 관점에서 B를 제시한다 | A하려고 B를 쓴다 / B로 A를 만든다 |

예:

```markdown
이 강의의 핵심은 AI 에이전트를 한 기계 안에 가두지 말자는 것이다.
Hermes에 스킬을 추가하고 성능을 높여도, 특정 PC에만 묶여 있으면 일상적으로 쓰기 어렵다.
그래서 강사는 Tailscale, Termius, tmux 조합으로 어디서든 접속 가능한 환경을 만든다.
```
...
- 압축된 한자어·전문어 → 독자가 바로 이해할 수 있게 풀기.
  - "고권한" / "고권" → "높은 권한"
  - "권한 상승" → 문맥에 따라 "권한이 높아짐" / "권한을 높임"

### F2. 운영 문서 구어체 순화

- 매뉴얼·강의노트·런북에서는 거칠거나 너무 구어적인 동사를 차분한 작업 동사로 바꾼다.
- 단, 장애·프로세스 종료를 정확히 말해야 하는 기술 문맥에서는 의미가 약해지지 않게 고른다.

| 원문 표현 | 윤문 예시 |
|-----------|----------|
| 연결이 끊기면 작업이 죽는다 | 연결이 끊기면 작업이 멈춘다 |
| 세션을 잡는다 | 세션을 설정한다 / 세션에 연결한다 |
| 포트를 잡는다 | 포트를 설정한다 / 포트를 연다 |
| 값을 때려 넣는다 | 값을 입력한다 / 값을 지정한다 |
...
- 핵심 동작·결론을 뒤로 미루는 종속절은 두괄식으로 분리한다. 독자가 먼저 할 일을 보게 한다.

| 원문 표현 | 윤문 예시 |
|-----------|----------|
| 세션을 완전히 끝내지 않고, 백그라운드로 분리합니다. | 세션을 백그라운드로 분리합니다. 세션을 끝내지 않습니다. |
| 설정을 바꾸지 않아도 되도록 기본값을 유지합니다. | 기본값을 유지합니다. 설정을 바꾸지 않아도 됩니다. |
